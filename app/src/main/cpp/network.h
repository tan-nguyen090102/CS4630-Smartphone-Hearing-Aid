//
// Created by yasin on 11/16/2023.
//

#ifndef HEARING_AID_APPLICATION_NETWORK_H
#define HEARING_AID_APPLICATION_NETWORK_H

#endif //HEARING_AID_APPLICATION_NETWORK_H
...
int valid_length()
{
    ...
    int idx;
    for (idx = 0; idx < LSTM_NUM_LAYERS; idx++)
    {
...
const int LSTM_NUM_SAMPLES = floor((VALID_LENGTH) / 64) - 1;
...
LSTM_Working_Memory* init_LSTM_Working_Memory(double init)
{
    ...
    wm->cell_state = createArray2D(LSTM_HIDDEN_SIZE, LSTM_BATCH_SIZE);
    wm->prev_cell_state = createArray2D(LSTM_HIDDEN_SIZE, LSTM_BATCH_SIZE);

    wm->hidden_states = (Array2D**) malloc(LSTM_NUM_SAMPLES * sizeof(struct Array2D*));
    ...
    fillArray2D(wm->cell_state, 0);
    fillArray2D(wm->prev_cell_state, 0);
...
void free_LSTM_Working_Memory(LSTM_Working_Memory* wm)
{
    ...
    freeArray2D(wm->cell_state);
    freeArray2D(wm->prev_cell_state);
