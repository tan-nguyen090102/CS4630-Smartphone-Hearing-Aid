...
Java_com_example_hearingaidapplication_MainActivity_runDenoiser(
        JNIEnv* env,
  ...
  // Run Denoiser
    ...
    //{
      ...
      //}
    //}
    //std::cout << getArray2DContents(lstmwm->input_sequence[0]) << std::endl;
    ...
    //std::cout << "Time: " << (end - start).count() << "ms" << std::endl;
    /*std::cout << "Batchs " << std::endl << std::endl;
    for (int i = 0; i < LSTM_NUM_SAMPLES; i++)
    {
      std::cout << "Batch " << i << ": " << getArray2DContents(lstm_out[i]) << std::endl;
    }
    ...
    free_LSTM_weights(lstmw);
    free_LSTM_Working_Memory(lstmwm);
